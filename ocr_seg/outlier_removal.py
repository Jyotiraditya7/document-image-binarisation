...
for cnt in contours:
    ...
    mean_stroke = int(np.mean(dist[dist > 0]) * 2) | 1
...
# Show before outlier removal ----
pre_img = img.copy()
for x1, y1, x2, y2 in merged_boxes:
    cv2.rectangle(pre_img, (x1, y1), (x2, y2), (0, 0, 255), 3)
cv2.imshow("4. Merged Contours (before outlier removal)", pre_img)
cv2.waitKey(0)
...
if n > 0:
    ...
    heights = np.array([(y2 - y1) for (x1, y1, x2, y2) in merged_boxes])
...
cv2.imshow("5. Final Merged Contours (Outliers Removed)", output)
cv2.waitKey(0)
