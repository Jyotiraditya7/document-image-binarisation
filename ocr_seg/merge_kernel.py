...
kernel_close = cv2.getStructuringElement(cv2.MORPH_RECT, (50,10))
...
# Merge boxes row-wise if they overlap in Y and are close in X
merged_boxes = []
for box in boxes:
    x1, y1, x2, y2 = box
    merged = False
    for i, mb in enumerate(merged_boxes):
        ...
        # --- New: vertical overlap condition ---
        vertical_overlap = min(y2, my2) - max(y1, my1)
        min_height = min(y2 - y1, my2 - my1)

        if vertical_overlap > 0 and vertical_overlap / min_height >= 0.5:
            # --- Also check horizontal closeness ---
            if abs(x1 - mx2) < 100 or abs(x2 - mx1) < 100:
                new_box = [
                    min(x1, mx1),
                    min(y1, my1),
                    max(x2, mx2),
...
cv2.imshow("Merged Side Boxes", output)
cv2.waitKey(0)
